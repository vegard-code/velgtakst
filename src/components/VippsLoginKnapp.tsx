 ...
 *
 * Bruk:
 *   <VippsLoginKnapp rolle="takstmann" redirect="/portal/takstmann" />
 *   <VippsLoginKnapp />  ← standard: privatkunde, redirect til /portal
 */

interface VippsLoginKnappProps {
  rolle?: 'takstmann_admin' | 'megler' | 'privatkunde'
  redirect?: string
  tekst?: string
}

export default function VippsLoginKnapp({
  rolle = 'privatkunde',
  redirect = '/portal',
  tekst = 'Logg inn med Vipps',
}: VippsLoginKnappProps) {
  function handleClick() {
    const params = new URLSearchParams({ rolle, redirect })
    window.location.href = `/api/auth/vipps?${params.toString()}`
  }
