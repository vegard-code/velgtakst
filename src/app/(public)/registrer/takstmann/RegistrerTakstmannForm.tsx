...
export default function RegistrerTakstmannForm() {
  ...
  const inputClass =
    "w-full bg-white border border-slate-200 text-slate-900 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-accent transition-colors placeholder-slate-400";
  const selectClass =
    "w-full bg-white border border-slate-200 text-slate-900 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-accent transition-colors";
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-8">
      ...
      <div className="flex items-center gap-3 my-6">
        ...
        <span className="text-xs text-slate-500 uppercase tracking-wider">eller manuelt</span>
        <div className="flex-1 h-px bg-slate-200" />
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {steg === 1 && (
          <>
            <h2 className="text-slate-800 font-semibold text-lg mb-4">Bedriftsinformasjon</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Firmanavn *</label>
                <input name="firmanavn" required className={inputClass} placeholder="Takst AS" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Organisasjonsnummer</label>
              ...
              <div className="sm:col-span-2">
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Faktura-epost *</label>
        ...
        {steg === 2 && (
          <>
            ...
            <h2 className="text-slate-800 font-semibold text-lg mb-1">Hva tilbyr du?</h2>
            <p className="text-slate-500 text-sm mb-5">Velg dine spesialiteter og andre tjenester du utfører.</p>

            {/* Spesialiteter */}
            <div className="space-y-3 mb-6">
              <label className="block text-sm font-medium text-slate-700">Spesialitet (velg opptil 2)</label>
            ...
            {/* Utfører også */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-3">Utfører også</label>
            ...
            <div className="flex gap-3 mt-4">
              <button
                type="button"
                onClick={() => setSteg(1)}
                className="flex-1 border border-slate-200 text-slate-600 hover:text-slate-900 hover:border-slate-400 font-medium py-3 rounded-lg transition-colors"
              >
                &larr; Tilbake
              </button>
              <button
        ...
        {steg === 3 && (
          <>
            ...
            <h2 className="text-slate-800 font-semibold text-lg mb-4">Din brukerkonto</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
              ...
              <div className="sm:col-span-2">
                <label className="block text-sm font-medium text-slate-700 mb-1.5">
                  Forbundstilknytning{" "}
                  <span className="text-slate-500 font-normal">(valgfritt)</span>
                </label>
                <select
                  name="sertifisering"
                  value={sertifisering}
            ...
            {(spes1 || valgteTjenester.length > 0) && (
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mt-2">
                <p className="text-xs text-slate-500 uppercase tracking-wide mb-2">Dine tjenester</p>
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {valgteTjenester
                    .filter((t) => t !== spes1 && t !== spes2)
                    .map((t) => (
                      <span key={t} className="text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full">{t}</span>
                    ))}
                </div>
              </div>
            )}
            ...
            <p className="text-xs text-slate-500">
              ...
              <a href="/vilkar" className="underline hover:text-slate-700">vilkår</a>
              {" "}og{" "}
              <a href="/personvern" className="underline hover:text-slate-700">personvernerklæring</a>.
            </p>
