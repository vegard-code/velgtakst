...
export default async function FinnTakstmannMeglerPage({ searchParams }: Props) {
  ...
  let query = supabase
    ...
    .select(`
      ...
      takstmann:takstmann_profiler!inner(
        id, navn, tittel, spesialitet, bio, telefon, epost, bilde_url, sertifiseringer
      )
    `)
    .eq("er_aktiv", true);
  ...
  if (spesialitet) {
    takstmenn = takstmenn.filter((t) =>
      t.spesialitet?.toLowerCase().includes(spesialitet.toLowerCase())
    );
  }
  if (sok) {
    takstmenn = takstmenn.filter((t) =>
      t.navn.toLowerCase().includes(sok.toLowerCase()) ||
      t.spesialitet?.toLowerCase().includes(sok.toLowerCase())
    );
  }
            ...
            <div key={t.id} className="portal-card portal-card-hover p-5">
              <div className="flex gap-4 items-start mb-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {t.spesialitet && (
                    <span className="inline-block mt-1 text-xs bg-[#e8f0f8] text-[#285982] px-2 py-0.5 rounded-full">
                      {t.spesialitet}
                    </span>
                  )}
                </div>
