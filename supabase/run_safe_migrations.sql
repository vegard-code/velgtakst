-- ============================================================
-- VelgTakst – SIKKER MIGRERING (oppretter kun det som mangler)
-- ============================================================

-- 001: TABLES (IF NOT EXISTS)
...
CREATE TABLE IF NOT EXISTS takstmann_profiler (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS fylke_synlighet (
  id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS megler_profiler (
  id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id        UUID UNIQUE REFERENCES auth.users(id) ON DELETE CASCADE,
  company_id     UUID REFERENCES companies(id) ON DELETE SET NULL,
  navn           TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS privatkunde_profiler (
  id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id     UUID UNIQUE REFERENCES auth.users(id) ON DELETE CASCADE,
  navn        TEXT NOT NULL,
  telefon     TEXT,
...
CREATE TABLE IF NOT EXISTS status_logg (
  id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS purre_logg (
  id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  oppdrag_id   UUID NOT NULL REFERENCES oppdrag(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS megler_vurderinger (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  takstmann_id  UUID NOT NULL REFERENCES takstmann_profiler(id) ON DELETE CASCADE,
  megler_id     UUID REFERENCES megler_profiler(id) ON DELETE SET NULL,
  oppdrag_id    UUID REFERENCES oppdrag(id) ON DELETE SET NULL,
...
-- TRIGGERS (CREATE OR REPLACE er trygt)
...
DROP TRIGGER IF EXISTS trg_takstmann_profiler_updated_at ON takstmann_profiler;
CREATE TRIGGER trg_takstmann_profiler_updated_at
  BEFORE UPDATE ON takstmann_profiler
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();

DROP TRIGGER IF EXISTS trg_oppdrag_updated_at ON oppdrag;
CREATE TRIGGER trg_oppdrag_updated_at
  BEFORE UPDATE ON oppdrag
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();

DROP TRIGGER IF EXISTS trg_bestillinger_updated_at ON bestillinger;
...
-- INDEXES
CREATE INDEX IF NOT EXISTS idx_takstmann_profiler_company ON takstmann_profiler(company_id);
CREATE INDEX IF NOT EXISTS idx_fylke_synlighet_fylke ON fylke_synlighet(fylke_id) WHERE er_aktiv = TRUE;
CREATE INDEX IF NOT EXISTS idx_oppdrag_company ON oppdrag(company_id);
CREATE INDEX IF NOT EXISTS idx_oppdrag_takstmann ON oppdrag(takstmann_id);
CREATE INDEX IF NOT EXISTS idx_oppdrag_status ON oppdrag(status);
CREATE INDEX IF NOT EXISTS idx_bestillinger_takstmann ON bestillinger(takstmann_id);
CREATE INDEX IF NOT EXISTS idx_status_logg_oppdrag ON status_logg(oppdrag_id);
CREATE INDEX IF NOT EXISTS idx_dokumenter_oppdrag ON dokumenter(oppdrag_id);
CREATE INDEX IF NOT EXISTS idx_megler_vurderinger_takstmann ON megler_vurderinger(takstmann_id);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'oppdrag_betalingsmetode_check'
  ) THEN
    ALTER TABLE oppdrag ADD CONSTRAINT oppdrag_betalingsmetode_check
...
ALTER TABLE user_profiles
  DROP CONSTRAINT IF EXISTS user_profiles_rolle_check,
