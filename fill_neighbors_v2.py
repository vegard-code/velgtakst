...
Behandler TypeScript-filen direkte ved å bruke regex-substitusjon.
"""
...
# For hver kommune i neighbors-dict, erstatt naerliggendeKommuner og naerliggendeText
updated_count = 0
for kommune_id, (neighbor_list, text) in neighbors.items():
    # Bygg JSON-array for nabokommuner
    neighbor_json = '[\n      ' + ',\n      '.join(f'"{n}"' for n in neighbor_list) + '\n    ]'

    # Regex for å finne og erstatte naerliggendeKommuner-arrayet for denne kommunen
    # Match: "kommune_id": { ... "naerliggendeKommuner": [...] ... }
    pattern = rf'(\"{kommune_id}\":.*?\"naerliggendeKommuner\": *)\[(.*?)\]'

    match = re.search(pattern, content, re.DOTALL)
    if match:
        content = re.sub(pattern, rf'\1{neighbor_json}', content, count=1, flags=re.DOTALL)
        updated_count += 1

    # Erstatt naerliggendeText
    text_pattern = rf'(\"{kommune_id}\":.*?\"naerliggendeText\": *)"[^"]*"'
    text_escaped = text.replace('"', '\\"')
    match = re.search(text_pattern, content, re.DOTALL)
    if match:
        content = re.sub(text_pattern, rf'\1"{text_escaped}"', content, count=1, flags=re.DOTALL)
        updated_count += 1
